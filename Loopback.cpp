////
// Created by Ordi on 12/11/2023.
//

#include "Loopback.h"

Loopback::Loopback(const BlackScholes& S, double T, double T0, int K, int N, int n) : S_(S), T_(T), T0_(T0), K_(K), N_(N), n_(n) {
    if (T < 0 || T0 < 0 || N <= 0 || n <= 0) {
...
const BlackScholes& Loopback::getS() const {
    return S_;
}

int Loopback::getT() const {
    return T_;
}

int Loopback::getT0() const {
    return T0_;
}

int Loopback::getK() const {
    return K_;
}

double Loopback::getPrice() const {
    return price_;
}

int Loopback::getn() const {
    return n_;
}
...
double Loopback::pricer(BlackScholes S, double T, double T0, int K, int N, int n) const {
...
double Loopback::delta(double epsilon) const {
...
double Loopback::gamma(double epsilon) const {
...
double Loopback::theta(double epsilon) const {
    int M = int(4 / std::pow(epsilon, 4));
...
double Loopback::rho(double epsilon) const {
    int M = int(4 / std::pow(epsilon, 4));
...
double Loopback::vega(double epsilon) const {
    int M = int(4 / std::pow(epsilon, 4));
